...
export default function Clinics() {
  ...
  const list = useQuery({
    queryKey: ['clinics', { search, clientFilter, bucketFilter, offset }],
    queryFn: () => {
  ...
  return (
    <div className="shell">
      ...
      <p className="muted">Global directory of every clinic across every client. Use the filters to narrow down.</p>
      ...
      <div className="card">
        <div className="row gap" style={{ flexWrap: 'wrap' }}>
          <input
            ...
            style={{ minWidth: 240 }}
          />
          <select value={clientFilter} onChange={(e) => { setOffset(0); setClientFilter(e.target.value); }}>
            ...
            <table className="tbl">
              ...
              <tbody>
                {list.data.clinics.length === 0 && (
                  <tr><td colSpan={5} className="muted center">No clinics match.</td></tr>
                )}
                {list.data.clinics.map((cl) => (
                  <tr key={cl.id} className={cl.is_active ? '' : 'dim'}>
                    <td><Link to={`/clinics/${cl.id}`}><strong>{cl.name}</strong></Link></td>
                    <td><Link to={`/clients/${cl.client_id}`} className="muted">{cl.client_name}</Link></td>
                    ...
                    <td className="right"><Link className="btn ghost" to={`/clinics/${cl.id}`}>Open</Link></td>
                  </tr>
                ))}
              </tbody>
            </table>
