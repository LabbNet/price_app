...
export default function ClientDetail() {
  ...
  const client = useQuery({ queryKey: ['client', id], queryFn: () => apiGet(`/api/clients/${id}`) });
  const clinics = useQuery({
    queryKey: ['clinics', { client_id: id, offset, search, bucketFilter }],
    queryFn: () => {
      ...
      return apiGet(`/api/clinics?${q.toString()}`);
    },
  });
  ...
  const createClinic = useMutation({
    mutationFn: (data) => apiPost('/api/clinics', { ...data, client_id: id }),
    onSuccess: () => { invalidate(); setCreatingClinic(false); },
  });

  const bulkImport = useMutation({
    mutationFn: (rows) => apiPost('/api/clinics/import', { client_id: id, clinics: rows }),
    onSuccess: () => { invalidate(); setImporting(false); },
  });

  const bulkAssign = useMutation({
    mutationFn: (bucket_id) => apiPost(`/api/clients/${id}/assign-bucket-to-all`, { bucket_id }),
    onSuccess: () => { invalidate(); setBulkAssigning(false); },
  });

  if (client.isLoading) return <div className="shell"><p className="muted">Loading…</p></div>;
  ...
  return (
    <div className="shell">
      <p className="muted"><Link to="/clients">← All clients</Link></p>
      ...
      <div className="row-between" style={{ marginTop: '1.5rem' }}>
        ...
        <div className="row gap">
          ...
          <button className="btn primary" onClick={() => setCreatingClinic(true)}>+ Add clinic</button>
        </div>
      </div>
      ...
      {clinics.isLoading && <p className="muted">Loading clinics…</p>}
      {clinics.isError && <p className="error">{String(clinics.error.message || clinics.error)}</p>}
            ...
            <table className="tbl">
              ...
              <tbody>
                {clinics.data.clinics.length === 0 && (
                  <tr><td colSpan={6} className="muted center">No clinics match.</td></tr>
                )}
                {clinics.data.clinics.map((cl) => (
                  <tr key={cl.id} className={cl.is_active ? '' : 'dim'}>
                    <td><Link to={`/clinics/${cl.id}`}><strong>{cl.name}</strong></Link></td>
                    ...
                    <td className="right">
                      <Link className="btn ghost" to={`/clinics/${cl.id}`}>Open</Link>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      {editing && (
        <ClientForm
          ...
          onSubmit={(data) => saveClient.mutate(data)}
          busy={saveClient.isPending}
          error={saveClient.error}
        />
      )}
      {creatingClinic && (
        <ClinicForm
          onCancel={() => setCreatingClinic(false)}
          onSubmit={(data) => createClinic.mutate(data)}
          busy={createClinic.isPending}
          error={createClinic.error}
        />
      )}
      {importing && (
        <ImportCsv
  ...
  return (
    <div className="modal" role="dialog">
      <div className="card modal-card" style={{ maxWidth: 780 }}>
        ...
        {result && <p className="muted">✓ Imported {result.imported} clinic(s).</p>}
...
function BulkAssign({ buckets, clinicCount, clientName, onSubmit, onCancel, busy, error, result }) {
  const [bucketId, setBucketId] = useState(buckets[0]?.id || '');
  return (
    <div className="modal" role="dialog">
      <div className="card modal-card">
        <h2>Bulk-assign bucket</h2>
        <p className="muted">Assigns the selected bucket to every active clinic under <strong>{clientName}</strong> ({clinicCount} clinic{clinicCount === 1 ? '' : 's'}). Prior assignments are closed.</p>
        ...
        {error && <p className="error">{String(error.message || error)}</p>}
        {result && <p className="muted">✓ Updated {result.updated} clinic(s).</p>}
        <div className="row gap end">
          <button type="button" className="btn ghost" onClick={onCancel}>Close</button>
