...
const baseSchema = z.object({
  ...
  condition_type: z.enum(['time_limited', 'single_order', 'client_specific']),
...
const updateSchema = baseSchema.partial().omit({ clinic_id: true, product_id: true });

// List with filters — useful both globally and scoped.
router.get('/', async (req, res) => {
  const limit = Math.min(Number(req.query.limit) || 100, 500);
  const offset = Math.max(Number(req.query.offset) || 0, 0);
  const { clinic_id, client_id, product_id, condition_type } = req.query;
  const active = req.query.active;
  const expired = req.query.expired === 'true';
  const exhausted = req.query.exhausted === 'true';

  const q = db('special_pricing as sp')
    .join('clinics as cl', 'cl.id', 'sp.clinic_id')
    .join('clients as c', 'c.id', 'cl.client_id')
  ...
  if (clinic_id) q.where('sp.clinic_id', clinic_id);
  if (client_id) q.where('c.id', client_id);
...
// Resolve effective price for (clinic, product) — the canonical lookup.
router.get('/resolve/:clinicId/:productId', async (req, res) => {
  const out = await resolveEffectivePrice({ clinicId: req.params.clinicId, productId: req.params.productId });
  res.json(out);
});

// Resolve all effective prices for a clinic — one per active product that has
// either a special row or a bucket row. Used by the clinic detail page.
router.get('/resolve-clinic/:clinicId', async (req, res) => {
  const clinic = await db('clinics').where({ id: req.params.clinicId }).first();
  ...
  for (const p of products) {
    const r = await resolveEffectivePrice({ clinicId: req.params.clinicId, productId: p.id });
    if (r.source !== 'none') {
      results.push({
        product_id: p.id,
        product_name: p.name,
  ...
  res.json({ clinic_id: clinic.id, effective: results });
});
