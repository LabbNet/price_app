...
/**
 * Resolve the currently-effective price for (clinic, product).
 *
 * Precedence:
 ...
 *        - condition_type = client_specific: active, no date/use bounds
 *      Preference order within specials: single_order > time_limited > client_specific,
 ...
 */
async function resolveEffectivePrice({ clinicId, productId, trx = db }) {
  ...
  const special = await trx('special_pricing')
    .where({ clinic_id: clinicId, product_id: productId, is_active: true })
    .andWhere((q) =>
      q
        .where((inner) =>
          inner
        ...
        .orWhere({ condition_type: 'client_specific' }),
    )
    .orderByRaw(`
      CASE condition_type
        WHEN 'single_order' THEN 1
        WHEN 'time_limited' THEN 2
        WHEN 'client_specific' THEN 3
      END
    `)
  ...
  // Fall through to the clinic's currently-assigned bucket.
  const bucketItem = await trx('bucket_items as bi')
    .join('clinic_bucket_assignments as cba', 'cba.bucket_id', 'bi.bucket_id')
    .where('cba.clinic_id', clinicId)
